				...
				<div class="card">
					...
					<div class="card-body">
						<table class="table">
							...
							<tr class="<?php echo($msg['status']==0?'bg-warning':''); ?>">
								<td><?php echo $sn++; ?></td>
								<td><?php echo $msg['name']; ?></td>
								<td><?php echo $msg['email']; ?></td>
								<td><?php echo substr($msg['message'], 0,25); ?>...</td>
								<td>
									<a href="view.php?id=<?php echo $msg['id']; ?>" class="btn btn-success btn-xs">View</a>
									<a href="tra.php?id=<?php echo $msg['id']; ?>" class="btn btn-danger btn-xs">Delete</a>
								</td>
							</tr>
						<?php endforeach; ?>
						<div class="mt-1">
                                            <?php if(!empty($_SESSION['trash'])): ?>
                                            <div class="alert alert-dismissable alert-warning">
                                              <button type="button" class="close" data-dismiss="alert">&times;</button>
                                              <?php echo $_SESSION['trash']; ?>
                                            </div>
                                        <?php endif; unset($_SESSION['trash']); ?>
                                    </div>
						<a href="trash.php" class="btn btn-warning">Trash</a><hr>
						</table>
					</div>
				</div>
			</div>
